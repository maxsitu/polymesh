...
class PolymeshGitUtil(db: H2MigrationDatabase)
  extends Git(
    db, System
      .getProperty("user.dir") + "/.git"
  ) {
  override def run(args: List[String]) {
    args match {
      case "install" :: Nil =>
        val currentDir = System
...
object GitUtil {
  private val config = ConfigFactory
    .load()
  val dbLoc = config
    .getString("slick.db.url")
  val objLoc = config
    .getString("slick.version_control_dir")
  ...
  def main(args: Array[String]) {
    ...
    val tool = new PolymeshGitUtil(db)
    tool
      .run(
        args
          .toList
